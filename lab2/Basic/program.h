...
public:
	...
	*/
	...
	void setParsedStatement(int &lineNumber, Statement *stmt, EvalState &state);

        /*
	* Method: runOneStatement
	* Usage: program.runOneStatement(input,stmt,state);
	* ----------------------------------------------------
	* run a statement without lineNumber immediately.
        */
	void runOneStatement(string input, Statement* stmt, EvalState& state);
...
private:
	map<int, std::string>store;
	map<int, std::string>::iterator storeIter;
	map<int, Statement*>parsed;
	map<int, Statement*>::iterator parsedIter;
};
